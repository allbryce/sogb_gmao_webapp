...
EASection.focusSearch = function() {
    SearchBox.Focus();
};

EASection.urrentDemoGroupKey = "";
EASection.CurrentDemoKey = "";
EASection.CurrentThemeCookieKey = "DXCurrentTheme";
EASection.CodeLoaded = false;
EASection.ShowCodeInNewWindow = function(demoTitle, codeTitle){
...
EASection.SetCurrentTheme = function(theme){
    ThemeSelectorPopup.Hide();
    ASPxClientUtils.SetCookie(EASection.CurrentThemeCookieKey, theme);
...
EASection.CodeNavBar_HeaderClick = function(s, e) {
    var source = _aspxGetEventSource(e.htmlEvent);
    var tag = source.tagName;
    e.cancel = tag != "A" && tag != "IMG";
}
EASection.ShowScreenshotWindow = function(evt, link, useExtendedPopup){
    EASection.ShowScreenshot(link.href, useExtendedPopup); 
    evt.cancelBubble = true; 
    return false;
}
EASection.ShowScreenshot = function(src, useExtendedPopup){
	var getPopupFunc = useExtendedPopup ? EASection.getScreenshotExtendedPopup : EASection.getScreenshotPopup;
    EASection.ShowScreenshotCore(src, getPopupFunc);
}
EASection.ShowScreenshotCore = function(src, getPopupFunc){
...
EASection.getScreenshotPopup = function(src, windowX, windowY, windowWidth, windowHeight, showScrollbars) {
...
EASection.getScreenshotExtendedPopup = function(src, windowX, windowY, windowWidth, windowHeight) {
    var popup = EASection.getScreenshotPopup("", windowX, windowY, windowWidth, windowHeight, true);
...
EASection.treeViewNodeClick = function(s, e) {
    var node = e.node;
    if(node.GetNavigateUrl() == "javascript:;") {
        node.SetExpanded(!node.GetExpanded());
    }
}
EASection.treeViewExpandedChanging = function(s, e) {
    var node = e.node;
...
EASection.Search = {
    ...
    onSearchBoxKeyPress: function(s, e) {
        ...
        if(text != EASection.Search.lastSearch) {
            if(EASection.Search.searchTimeout != null)
                clearTimeout(EASection.Search.searchTimeout);
            EASection.Search.searchTimeout = setTimeout(function() {
                EASection.Search.doSearch(text);
            }, 250);
        }
        EASection.Search.lastSearch = text;
    },
    onSearchBoxKeyDown: function(s, e) {
        ...
        if(keyCode == 13) {
            ...
            if(EASection.Search.selectedItem)
                document.location = EASection.Search.selectedItem.GetNavigateUrl();
        }
        if(keyCode == 40) {
            EASection.Search.moveSelectedItem(1);
            prevent = true;
        }
        if(keyCode == 38) {
            EASection.Search.moveSelectedItem(-1);
    ...
    onSearchBoxGotFocus: function() {
        ...
        if(text && text.length > 2)
            EASection.Search.showPopup();
    },

    doSearch: function(text) {
        EASection.Search.selectedItem = null;
        if(text && text.length > 2) {
            EASection.Search.showPopup();
    ...
    onSearchPopupBeginCallback: function() {
        EASection.Search.isInCallback = true;
    },
    onSearchPopupEndCallback: function() {
        EASection.Search.isInCallback = false;
    },
    moveSelectedItem: function(direction) {
        ...
        var group, item;
        var newItem = null;
        if(groupCount == 0 || !SearchPopup.IsVisible() || EASection.Search.isInCallback) {
            EASection.Search.selectedItem = null;
            return;
        }
        if(EASection.Search.selectedItem == null) {
            group = SearchResultsNavBar.GetGroup(direction > 0 ? 0 : groupCount - 1);
            item = group.GetItem(direction > 0 ? 0 : group.GetItemCount() - 1);
            newItem = item;
        }
        else {
            group = EASection.Search.selectedItem.group;
            if(direction > 0) {
                if(EASection.Search.selectedItem.index < group.GetItemCount() - 1)
                    newItem = group.GetItem(EASection.Search.selectedItem.index + 1);
                else {
                ...
                if(EASection.Search.selectedItem.index > 0)
                    newItem = group.GetItem(EASection.Search.selectedItem.index - 1);
                else {
        ...
        EASection.Search.selectNavBarItem(newItem);
    },
    showPopup: function() {
        EASection.Search.selectNavBarItem(null);
        SearchPopup.ShowAtElementByID("SearchPanel");
    },
    selectNavBarItem: function(item) {
        if(window.SearchResultsNavBar && SearchResultsNavBar.GetMainElement()) {
            SearchResultsNavBar.SetSelectedItem(item);
            EASection.Search.selectedItem = item;
            if(item) {
...
EAEventMonitor = {
    ...
    Clear: function() {
       EAEventMonitor.GetLogElement().innerHTML = ""; 
    },
...
DXUploadedFilesContainer = {
    ...
    BuildLink: function(builder, text, url) {
        builder.push("<a target='blank' onclick='return EASection.ShowScreenshotWindow(event, this, " + this.useExtendedPopup + ");'");
...
EASection.HightlightedCode = {
    ...
    StartScrollAnimation: function(from, to, documentScrollY) {
        ...
        ASPxAnimationHelper.createSimpleAnimationTransition({
            duration: duration,
            transition: ASPxAnimationConstants.Transitions.SINE,
            onUpdate: function (value) { 
                documentScrollY += value;
                if(__aspxMacOSMobilePlatform) // TODO: B157267
                    document.body.scrollTop = documentScrollY;
                else
                    _aspxSetDocumentScrollTop(documentScrollY);
            }
        }).Start(from, to);
    },

    DemoPageControl_OnActiveTabChanged: function(s, e) {
        if(!EASection.HightlightedCode.visitedTabArrayIndexes[s.GetActiveTabIndex()]) {
            if(e.tab.name != "Description") {
                var pageControlElement = DemoPageControl.GetMainElement(),
                    topElementOffset = EASection.HightlightedCode.GetPageControlOffsetTop(),
                    topScreenOffset = EASection.HightlightedCode.TOP_OFFSET_FOR_TAB_CONTROL,

                    scrollDistance = 0,
                    documentScrollY = 0,
                    scrollTop = __aspxMacOSMobilePlatform ? document.body.scrollTop : _aspxGetDocumentScrollTop(); // TODO: B157267
                ...
                var bottomOfScreenOnPage = scrollTop + window.innerHeight + EASection.HightlightedCode.BOTTOM_PADDING_FOR_CODE;

                if(pageControlBottomHeight > bottomOfScreenOnPage)
                    EASection.HightlightedCode.StartScrollAnimation(scrollTop, scrollDistance, documentScrollY);
            }
            EASection.HightlightedCode.visitedTabArrayIndexes[s.GetActiveTabIndex()] = true;
        }
    }
};
...
EASection.HightlightedCode = {
    ...
    DemoPageControl_OnActiveTabChanged: function(s, e) {
        if(!EASection.HightlightedCode.visitedTabArrayIndexes[s.GetActiveTabIndex()]) {
            if(e.tab.name != "Description") {
                var isASPXtab = e.tab.name == "ASPX" || EASection.HightlightedCode.GetFileExtension(e.tab.name) == "aspx",
                ...
                if(isASPXtab) {
                    var highlightedBlocks = _aspxGetDescendantNodesByClassName(document.documentElement, "HighlightedBlock");
                    if(highlightedBlocks.length > 0) {
                        topElementOffset = _aspxGetAbsoluteY(highlightedBlocks[0]);
                        elementOffsetHeight = highlightedBlocks[0].offsetHeight;
                        topScreenOffset = EASection.HightlightedCode.TOP_OFFSET_FOR_HIGHLIGHTED_BLOCK;
                        highlightedDivOffsetY = topElementOffset;
                    }
                }

                if(topElementOffset - EASection.HightlightedCode.GetPageControlOffsetTop() + EASection.HightlightedCode.BOTTOM_PADDING_FOR_ASPX_CODE < window.innerHeight) {
                    topElementOffset = EASection.HightlightedCode.GetPageControlOffsetTop();
                    topScreenOffset = EASection.HightlightedCode.TOP_OFFSET_FOR_TAB_CONTROL;
                ...
                var pageHeightBelowScreen = document.body.scrollHeight - scrollTop - window.innerHeight;
                var endCodeContainer = pageControlElement.offsetTop + pageControlElement.offsetHeight - window.innerHeight;
                if(elementOffsetHeight < window.innerHeight && highlightedDivOffsetY - EASection.HightlightedCode.GetPageControlOffsetTop() > window.innerHeight)
                    scrollDistance = highlightedDivOffsetY + elementOffsetHeight - window.innerHeight + EASection.HightlightedCode.SCROLL_CODE_BOTTOM_PADDING;
                else
                    scrollDistance = topElementOffset - topScreenOffset > pageHeightBelowScreen ? endCodeContainer : topElementOffset - topScreenOffset;
